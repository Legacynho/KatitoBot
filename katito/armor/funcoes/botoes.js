...
var blackchannel = JSON.parse(fs.readFileSync("./dono/config-all.json")).blackchannel
  ...
  try {
    if(botoes) {
      ...
      for(i of dados) {
        await atraso(1500)
        if(i?.image) {
          img = await prepareWAMessageMedia({image: i?.image}, {upload: blackmd.waUploadToServer})
          ...
          vid = await prepareWAMessageMedia({video: i?.video}, {upload: blackmd.waUploadToServer})
      ...
      blackmd.relayMessage(from, {interactiveMessage: {carouselMessage: {cards: butao}}}, {})
    } else {
      for(i of dados) {
        if(i?.image) {
          await atraso(1000)
